...
import { AlertTriangle, Info, CircleDashed } from "lucide-react";
...
export default function PortfolioVaRChart({ 
  ...
  useEffect(() => {
    try {
      ...
      console.error('Error processing portfolio VaR time-series data:', error);
      setChartData([]);
    }
  }, [varTimeSeriesData]);
  ...
  if (varTimeSeriesLoading) {
    return (
      <CardGradient className="h-[400px] flex items-center justify-center">
        <p className="text-muted-foreground">Loading portfolio VaR analysis...</p>
      </CardGradient>
    );
  }
  ...
  if (prices.length === 0) {
    return (
      <CardGradient className="h-[400px] flex items-center justify-center">
        <p className="text-muted-foreground">Invalid price data for portfolio</p>
      </CardGradient>
    );
  }
